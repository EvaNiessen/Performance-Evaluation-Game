...
def main():
    ...
    logfile2.write("block \t trial_nr \t trial_type \t RT \t miss \t chosen_button \t correct_button \t evaluation \t accuracy \t RT_eval \t category_left \t color_left \t category_right \t color_right \r\n")
    for i in range(int(configs['General']['block_number'])):
        ...
        for j in range(int(configs['Block']['main_length'])):
            ...
            if guessed_number[1] > -1: #Info fuer logfile wenn Antwort gegeben wurde
                logfile.write("{0} \t {1} \t  {2} \t {3:04.0f} \t 0 \t {4} \t {5} \t {6} \t {7} \t {8:04.0f} \t {9} \t {10} \t {11} \t {12} \r\n".format(
                i+1,j+1,block_procedure[j][0],guessed_number[1], guessed_number[0]+1,gui.pictures.position+1, 4-eval_button, int(guessed_number[0]==gui.pictures.position),eval_time,
                gui.pictures.last_shown_images[0][0]+1, gui.pictures.last_shown_images[0][1]+1,gui.pictures.last_shown_images[1][0]+1, gui.pictures.last_shown_images[1][1]+1 ))
            else: #Info fuer logfile bei timeout
                logfile.write("{0} \t {1} \t {2} \t 0 \t 1 \t {3} \t {4} \t {5} \t 3 \t {6:04.0f} \t {7} \t {8} \t {9} \t {10}\r\n".format(
                i+1,j+1,block_procedure[j][0],choosen+1,gui.pictures.position+1,4-eval_button, eval_time,
                gui.pictures.last_shown_images[0][0]+1, gui.pictures.last_shown_images[0][1]+1,gui.pictures.last_shown_images[1][0]+1, gui.pictures.last_shown_images[1][1]+1))
