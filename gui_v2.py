...
# -*- coding: utf-8 -*-
import rules_v1
from bildgenerator_v2 import Bildgenerator
...
class Gui:
    ...
    def __init__(self, input_keys, highscore,picture_size, evaluation_time, color_scheme):
        ...
        self.second_training = True
        # Setze global key zum Beenden
        event.globalKeys.add(key='e', func=self.shutdown)

    # Schliesse Fenster mit global key 'e'
    def shutdown(self):
        self.win.close()
        core.quit()
    ...
    def show_pictures(self, trial):
        aim_vertices =self.picture_size/15
        aim = visual.ShapeStim(win = self.win ,units = 'pix', lineColor = [255,255,255],
        ...
        aim_black = visual.ShapeStim(win = self.win ,units = 'pix', lineColor = [0,0,0],
                               vertices = ((-aim_vertices,aim_vertices),(-aim_vertices,-aim_vertices),
                               (aim_vertices,-aim_vertices),(aim_vertices,aim_vertices)),pos=(0,self.picture_size))
    ...
    def show_intertrial(self, choosen):
        ...
        self.win.flip()
        time.sleep(0.5)
        self.text.setText("War das gewählte Bild richtig?")
    ...
    def show_introduction(self):
        ...
        if self.first_training:
            self.text.setText("Übung starten?")
        else:
            self.text.setText("Erneuter Durchgang der Übung?")
        self.text.draw()
        
        # Speichere die vorherige Textposition
        prev_pos = self.text.pos
        # Setze Instruktion unterhalb der Schaltflächen
        self.text.pos= (0,-2*self.picture_size)
        self.text.setText("Taste 'y'            Taste 'n'")
        self.text.draw()
        # Setze Textposition zurück
        self.text.pos = prev_pos
        
        self.texts[1].text = "Ja" # Setzt den Text zweier Evaluationsbuttons auf 'yes' und 'no'
        # Passe Texthöhe an um "Training überspringen" unterzubringen
        self.texts[2].height = self.picture_size/7
        self.texts[2].text = "Übung überspringen"
        for i in range(1,3):
            self.shapes[i].lineColor = (0.1,0.9,1) # farbe anders
            self.shapes[i].lineWidth = 3
            self.shapes[i].draw()
        ...
        self.texts[2].height = self.picture_size/5
        self.texts[2].text = "Vielleicht nicht"
        
        # Markiere Training als durchgeführt
    ...
    def show_tryout(self):
        '''Asks whether another tryout block should be started'''
        if self.second_training:
            self.text.setText("Training starten?")
        else:
        ...
        self.texts[1].text = "Yes" # Setzt den Text zweier Evaluationsbuttons auf 'yes' und 'no'
        # Passe Texthöhe an um "Training überspringen" unterzubringen
        self.texts[2].height = self.picture_size/7
        self.texts[2].text = "Training überspringen"
        for i in range(1,3):
            self.shapes[i].lineColor = (0.1,0.9,1) # farbe anders
            self.shapes[i].lineWidth = 3
            self.shapes[i].draw()
    ...
    def show_begin_of_block(self, pause, max, counter):
        ...
        for i in range(pause):
            self.score_anzeige.setText("Score: "+str(self.score)+ " / "+str(max))
            self.score_anzeige.draw()
            self.blocksleft.setText("Verbleibende Durchgänge: "+str(counter))
        ...
        self.blocksleft.draw()
        self.highscore_anzeige.draw()
        self.text.setText("Drücken Sie eine Taste um weiter zu machen")
        self.text.draw()
        self.win.flip()
        event.waitKeys(keyList = self.keys)
    ...
    def show_rules(self):
        ...
        image_display = visual.ImageStim(self.win, units = 'pix', pos=(0, 1.5*self.picture_size),
                                size = (self.picture_size*6, self.picture_size*4))
...
#    def show_rules_again(self):
#        '''Asks whether another introduction block should be started'''
#        self.text.setText("Wollen Sie die Spielregeln erneut lesen?")
#        self.text.draw()
#        
#        # Speichere die vorherige Textposition
#        prev_pos = self.text.pos
#        # Setze Instruktion unterhalb der Schaltflächen
#        self.text.pos= (0,-2*self.picture_size)
#        self.text.setText("Taste 'j'            Taste 'n'")
#        self.text.draw()
#        # Setze Textposition zurück
#        self.text.pos = prev_pos
#        
