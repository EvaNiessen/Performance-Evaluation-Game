...
# -*- coding: utf-8 -*-
import rules
from bildgenerator import Bildgenerator
...
class Gui:
    ...
    def create_textshapes_and_pictures(self):
        '''Creates the textshape for highscorescreen and the pictures wich could be selected'''
        for i in range(0,4):
    ...
    def show_right(self, choosen):
        '''Shows the right and the choosen picture for introduction Block; after this the self-evaluation screen is shown'''
        for i in range(4):
    ...
    def show_intertrial(self, choosen):
        '''Returns the button wich is pressed in the self-evaluation screen, returns -1 if no key is pressed'''
        for i in range(4):
            if i==choosen:
                self.shapes[i].lineColor = (-1,-1,1)        #Umrandet das gewaehlte Bild mit einem gruenen Kasten
                self.shapes[i].lineWidth = 5
                self.shapes[i].draw()
            else:
                self.image[i].opacity = 0.5                 #Alle anderen Bilder werden ausgeblasst
        ...
        self.text.draw()
        for i in range(0,4):
            self.shapes[i].lineColor = (-1,-1,1)
            self.shapes[i].lineWidth = 1.5
            self.shapes[i].draw()
            self.texts[i].draw()
        self.win.flip()
        key_input = []
        ...
        if key_input:
            return self.keys.index(key_input[0])
        return -1                                               #Returnt -1 falls keine Eingabe stattfindet
    ...
    def show_introduction(self):
        '''Asks wether another introduction block should be started'''
        self.text.setText("Weiter mit Einleitung?")
        self.text.draw()
    ...
    def show_tryout(self):
        '''Asks wether another tryout block should be started'''
        self.text.setText("Weiter mit Training?")
        self.text.draw()
        self.texts[1].text = "Ja" # Setzt den Text zweier Evaluationsbuttons auf 'yes' und 'no'
        self.texts[2].text = "Nein"         
        for i in range(1,3):
    ...
    def show_begin_of_block(self, pause, max, counter):
        ''' Shows the  highscore, own score and the maximum possible score at the begin of each block, 
        pause is the time the proband minimum has to wait before he can start the next block'''
    ...
    def which_button(self, stoptime):
        '''Returns the button wich is pressed while Gamescreen'''
        event.getKeys()         #Leert den Eingabebuffer
        starttime = time.time()
    ...
    def show_rules(self):
        ...
        while rules.content:
            text_display.setText(rules.content[0][0])   #Setzt den Text auf den als erstes im Tupel angegebenen String
            image_display.setImage(rules.content[0][1]) #Setzt das Bild auf die als zweites im Tupel angegebene Quellldatei
            del rules.content[0]
            text_display.draw()
            image_display.draw()
            self.win.flip()
